...
// Get judge's active assignments
router.get('/assignments', auth, async (req, res) => {
    try {
        ...
        const assignments = await Promise.all(activeHackathons.map(async (hackathon) => {
            ...
            const reviewedSubmissions = submissions.filter(s => s.reviews.some(r => r.judge.toString() === judge._id.toString()));
...
router.get('/upcoming-hackathons', auth, async (req, res) => {
    try {
        ...
        // Check if judge has already requested to judge each hackathon
...
router.post('/request-to-judge/:hackathonId', auth, async (req, res) => {
    try {
        ...
        if (judge.requestedHackathons?.some(req => req.hackathon.toString() === hackathon._id.toString())) {
            return res.status(400).json({ message: 'Already requested to judge this hackathon' });
        }

        judge.requestedHackathons = judge.requestedHackathons || [];
...
router.post('/respond-to-invite/:hackathonId', auth, async (req, res) => {
    try {
        ...
        const invitation = judge.invitedHackathons.find(
            inv => inv.hackathon.toString() === hackathon._id.toString()
        );
        ...
        if (response === 'accept') {
            // Add judge to hackathon's judges array if accepted
